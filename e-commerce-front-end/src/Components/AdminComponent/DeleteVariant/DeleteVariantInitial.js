...
function DeleteVariantInitial() {
    ...
    const navigateToItem = (productId, variantId) => {
        navigate(`/admin/delete-variant/${productId}/${variantId}`)
    }
            ...
            {returnedItems.attributes.length>1 ?
              <div>         
                ...
                {returnedItems.attributes.map((elemAttribute, index) =>(                
                    <ColoredCircle key={index} backgroundColor={elemAttribute.color} onClick={() => navigateToItem(returnedItems._id, elemAttribute._id)}></ColoredCircle>                
                ))}
                </NewVarianColoredCircleRow>              
              </div>
...
export default DeleteVariantInitial